...
$CLUSTER_1="aks-1"
$RESOURCE_GROUP_1="rg-aks-1"

$CLUSTER_2="aks-2"
$RESOURCE_GROUP_2="rg-aks-2"
...
az group create --name $RESOURCE_GROUP_VAULT --location westeurope
...
# az backup policy create -n "aks-backup-policy" -g $RESOURCE_GROUP_VAULT `
#    --vault-name $BACK_VAULT_NAME `
...
az backup container register --resource-group $RESOURCE_GROUP_2 --vault-name $BACK_VAULT_NAME --subscription $SUBSCRIPTION_ID --backup-management-type AzureKubernetesService --workload-type AzureKubernetesService --query properties.friendlyName -o tsv

$CONTAINER_NAME=$(az backup container list --resource-group $RESOURCE_GROUP_2 --vault-name $BACK_VAULT_NAME --subscription $SUBSCRIPTION_ID --backup-management-type AzureKubernetesService --query "[0].name" -o tsv)

az backup item set-policy --resource-group $RESOURCE_GROUP_2 --vault-name $BACK_VAULT_NAME --subscription $SUBSCRIPTION_ID --container-name $CONTAINER_NAME --item-name $CONTAINER_NAME --policy-name "aks-backup-policy"
